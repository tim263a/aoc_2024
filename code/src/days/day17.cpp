...
std::vector<int64_t> Day17::simulate(bool abortOnOut)
{
...
uint64_t Day17::testFor2(uint64_t prefix, std::size_t backIdx)
{
    if (backIdx >= m_opCodes.size())
    {
        return prefix;
    }
    ...
    {
        uint64_t candidateA = (prefix << 3ULL) + i;

        m_regA = candidateA;
        m_regB = m_origB;
        m_regC = m_origC;

        uint64_t output = simulate(true)[0];
        uint64_t targetNumber = m_opCodes.rbegin()[backIdx];
        ...
        uint64_t found = testFor2(candidateA, backIdx + 1);
        if (found)
        {
            return found;
        }
